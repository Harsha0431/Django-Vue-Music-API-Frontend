...
const handleLikeTrackClick = async () => {
    if (spotifyStore.liked_list.includes(spotifyStore.track_list[spotifyStore.current_track])) {
        const index = spotifyStore.liked_list.indexOf(
            spotifyStore.track_list[spotifyStore.current_track]
        )
        if (index > -1) {
            isLikeInProgress.value = true
            await removeFromLikedListService(
                { track_id: spotifyStore.track_list[spotifyStore.current_track] },
                userStoreObj.userToken
            )
                .then((res) => {
                    if (res.code == 1) {
...
watchEffect(() => {
    if (toastStoreObj.showAcknowledgement) {
        setTimeout(() => {
            toastStoreObj.showAcknowledgement = false
        }, 5000)
    }
})
</script>
...
<template>
    <div class="w-full sticky z-50 dark:bg-[#18171f]">
        <div
            v-if="toastStoreObj.showAcknowledgement"
            :class="[
                toastStoreObj.showAcknowledgement ? 'translate-y-0' : '-translate-y-24 hidden'
            ]"
            class="transition-all duration-1000 px-2 py-2 bg-[#ff3956] text-center rounded-ee-full rounded-es-full"
        >
            <span class="text-[#fff] tracking-wider max-ssm:text-[14px] max-vsm:text-[13px]">{{
                toastStoreObj.acknowledgementMessage
            }}</span>
        </div>
        <!-- Try nav color with #211e2b -->
        <Disclosure
            as="nav"
            class="navbar dark:bg-[#17161f] bg-white border-b-2 dark:border-b-0 shadow-sm dark:shadow-none"
            v-slot="{ open }"
        >
            <div class="mx-auto sm:max-w-7xl px-2 sm:px-6 lg:px-8">
                <div class="relative flex h-16 items-center justify-between">
                    ...
                    <div class="relative inset-y-0 left-0 flex items-center sm:hidden">
                        <!-- Mobile menu button-->
                        <DisclosureButton
                            class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:text-gray-700 dark:hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-gray-700 dark:focus:ring-white"
                        >
                    ...
                    <div class="flex items-center justify-center">
                        <div class="flex justify-center gap-4 sm:gap-8 md:gap-12">
                            <div class="hidden sm:flex gap-2 md:gap-4">
                                <RouterLink
                                    to="/"
                                    active-class="active-nav-icon"
                                    class="nav-icon text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
                                    >Home</RouterLink
                                >
                                <RouterLink
                                    to="/library"
                    ...
                    <div class="relative inset-y-0 right-0 flex items-center gap-2">
                        <button @click="toggleTheme" class="ml-auto relative p-1">
                            <svg
                                v-if="!themeStoreObj.isDarkTheme"
                                class="w-6 fill-gray-500 hover:fill-gray-700 dark:fill-gray-400 dark:hover:fill-gray-300"
                                viewBox="0 0 35 35"
                                data-name="Layer 2"
                                id="Layer_2"
                                xmlns="http://www.w3.org/2000/svg"
                            ...
                            <svg
                                v-else
                                class="w-6 fill-gray-300 hover:fill-gray-100"
                                viewBox="0 0 24 24"
                                fill="none"
                                xmlns="http://www.w3.org/2000/svg"
                            >
                        ...
                        <Menu
                            as="div"
                            v-if="userStoreObj.isLoggedIn"
                            class="relative inline-block text-left"
                        >
                            <MenuButton>
                                <button
                                    class="flex justify-center align-middle rounded-full avatar dark:bg-[#2d2d2d]"
                                >
                                    <span
                                        class="font-semibold px-[10px] text-lg py-[3px] text-white"
                                        >{{
                                            userStoreObj.isLoggedIn
                                                ? userStoreObj.username[0].toUpperCase()
                                                : 'G'
                                        }}</span
                                    >
                                </button>
                            ...
                            >
                                <MenuItems
                                    class="absolute py-2 right-0 z-10 mt-2 w-max origin-top-right rounded-md dark:bg-gray-600 bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                                >
                                    <div class="py-1 px-3">
                                        <MenuItem v-slot="{ active }">
                                            <button
                                                :class="[
                                                    active
                                                        ? 'dark:bg-gray-700 dark:text-gray-100 bg-gray-200'
                                            ...
                                            <button
                                                ...
                                                :class="[
                                                    active
                                                        ? 'dark:bg-gray-700 dark:text-gray-100 bg-gray-200'
                                                        : 'text-gray-700 dark:text-gray-200',
                                                    'block text-left w-full px-4 py-2 text-sm rounded-lg'
                                                ]"
                                            >
                                                Switch Theme
                                            </button>
                        ...
                        <button v-else>
                            <RouterLink
                                to="/login"
                                class="font-bold text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300"
                                >Login</RouterLink
                            >
                        </button>
                    </div>
            ...
            <DisclosurePanel class="sm:hidden flex justify-center">
                <div
                    class="space-y-1 px-2 pb-3 pt-2 text-center w-min flex flex-col justify-center"
                >
                    <RouterLink
                        ...
                        as="a"
                        :to="item.link"
                        active-class="mobile-active-nav-icon"
                        :class="[
                            'text-gray-500 hover:text-gray-700 dark:text-gray-400',
                            'block text-center rounded-md px-4 py-2 text-base font-medium'
                        ]"
        ...
        <div
            v-if="userStoreObj.isLoggedIn"
            class="w-full relative vsm:ml-3 pb-2 mt-2 flex justify-center"
        >
            <div
                class="flex dark:bg-[#2d2d2d] bg-[#efefefc4] relative ssm:ml-6 py-2 ssm:px-10 px-6 gap-4 rounded-2xl"
            >
                <div class="flex gap-4 justify-evenly">
                    <button
                        @click="() => handleTrackChange('prev')"
                        class="prev-btn stroke-[#ef4b56] transition-colors hover:stroke-[#ef4b56f4] opacity-75 hover:opacity-100"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                    ...
                    <button disabled class="stroke-[#ef4b56] cursor-not-allowed">
                        <svg
                            v-if="spotifyStore.is_active && spotifyStore.isPlaying"
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke-width="1.5"
                    ...
                    <button
                        ...
                        class="skip-btn stroke-[#ef4b56] transition-colors hover:stroke-[#ef4b56f4] opacity-75 hover:opacity-100"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                ...
                <button
                    class="relative active:animate-ping ml-4"
                    :class="[isLikeInProgress ? 'animate-ping' : '']"
                    @click="handleLikeTrackClick"
                >
                    <svg
                        v-if="
                            spotifyStore.liked_list.includes(
                                spotifyStore.track_list[spotifyStore.current_track]
                            )
                        "
                        xmlns="http://www.w3.org/2000/svg"
                        viewBox="0 0 24 24"
                        class="w-6 h-6 like-icon-filled fill-red-600"
                    >
                        <path
                    ...
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        stroke-width="1.5"
                        class="w-6 h-6 lik-icon-outline stroke-[#ef4b56f4] hover:animate-pulse hover:stroke-[#ef4b56]"
                    >
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
                ...
                <button @click="handleAddToPlaylistClick">
                    <svg
                        ...
                        viewBox="0 0 24 24"
                        stroke-width="1.5"
                        class="stroke-[#ef4b56] transition-colors hover:stroke-[#ef4b56f4] w-6 h-6"
                    >
                        <path
                            stroke-linecap="round"
                            stroke-linejoin="round"
