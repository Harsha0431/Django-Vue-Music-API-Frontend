...
const playSelectedTrack = (id) =>{
    if(spotifyStore.isPlaying)
        spotifyStore.playSelectedTrack(id)
    else{
    ...
    <div class="flex w-full h-full justify-center overflow-y-auto">
        <div class="flex h-full w-full justify-center pr-2">
            <div class="flex gap-x-4 flex-wrap gap-y-3 justify-start relative">
                <div v-for="(track , index) in props.list" :key="index"
                    class="group px-4 justify-center place-items-center rounded-md py-2 flex gap-x-4 h-fit w-full overflow-hidden
                            dark:bg-[#39374267] bg-[#efefef]
                            dark:hover:bg-[#2a2931] hover:bg-[#e8e8e8] transition-all z-0"
                >
                    <div class="w-[50px] h-[50px] md:w-[60px] md:h-[60px] overflow-hidden rounded-md">
                        <img
                    ...
                    <div class="flex justify-between place-items-center w-full gap-x-2">
                        ...
                        <div class="md:opacity-0 flex transition-all justify-center place-items-center group-hover:opacity-100">
                            <button class="rounded-full z-[100] h-fit relative bg-[#ff3957e2] transition-all p-2 hover:bg-[#ff3957]">
                                ...
                                <svg @click="()=>playSelectedTrack(track.track_id)" v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" class="stroke-gray-200 play w-6 h-6">
