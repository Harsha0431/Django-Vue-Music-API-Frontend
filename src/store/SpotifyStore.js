...
import { ref } from 'vue'
import {
    addInterestedListToDbService,
    fetchRecommendedTracksService
} from '@/service/recommendations/interestedListService'
...
export const useSpotifyStore = defineStore('Spotify Store', () => {
    ...
    const track_list = ref([
        ...
        'spotify:track:7ErkrGvaAbLWgWS7wJ4D7f',
    ...
    const addTrackToInterestedList = async (newValue) => {
        if (
            completely_listened_list.value.includes(newValue) &&
            !liked_list.value.includes(newValue) &&
            !interested_list_from_model.value.includes(newValue)
        ) {
            interested_list.value.unshift(newValue)
            if (interested_list.value.length > 0) {
                await addInterestedListToDbService({ track_list: interested_list.value }).then(
                    (res) => {
                        if (res.code == 1) {
    ...
    const nextTrack = async () => {
        if (track_list.value.length > 0) {
            ...
            if (current_playing.value == 'liked' || current_playing.value == 'interested')
                ...
                current_track.value =
                    Math.floor(Math.random() * track_list.value.length) % track_list.value.length
        ...
        await fetchRecommendedTracksService().then((res) => {
            if (res.code == 1) {
                ...
                res.data.forEach((track) => {
                    if (!previous_list.value.includes(track)) track_set.add(track)
                })
    ...
    const setCurrentPlayingList = async (type) => {
        if (type == 'liked') {
            ...
            if (recommendation_list.value.length > 0) {
                ...
                if (recommendation_list.value.length > 0) {
                    track_list.value = recommendation_list.value
                } else await fetchRecommendedTracks_from_db('custom')
            } else await fetchRecommendedTracks_from_db('custom')
        }
